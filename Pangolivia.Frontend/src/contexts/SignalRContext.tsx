...
export function SignalRProvider({ children }: { children: React.ReactNode }) {
  ...
  const [finalResults, setFinalResults] = useState<FinalGameRecord | null>(
    null
  );
  const [answerSubmitted, setAnswerSubmitted] = useState(false);
  const lastQuestionRef = useRef<QuestionForPlayer | null>(null);
  ...
  useEffect(() => {
    ...
    connection.on("receiveroundresults", (results: RoundResults) => {
      // Attach the answer options from the last question to the results
      const enrichedResults = {
        ...results,
        answer1: lastQuestionRef.current?.answer1,
        answer2: lastQuestionRef.current?.answer2,
        answer3: lastQuestionRef.current?.answer3,
        answer4: lastQuestionRef.current?.answer4,
      };
      setRoundResults(enrichedResults);
    });
